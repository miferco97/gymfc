...
class POS_VEL_PID_REAL():
    def __init__(self, env, kp_R_pos, kp_P_pos, kp_Y_pos, kp_R_vel, ki_R_vel, kd_R_vel, kp_P_vel, ki_P_vel, kd_P_vel, kp_Y_vel, ki_Y_vel, kd_Y_vel, f=70.0):
        ...
        self.ki_R_vel = ki_R_vel / 100.0
        self.kd_R_vel = kd_R_vel * 100.0

        # Pitch PID constants
        self.kp_P_vel = kp_P_vel
        self.ki_P_vel = ki_P_vel / 100.0
        self.kd_P_vel = kd_P_vel * 100.0

        # Yaw PID constants
        self.kp_Y_vel = kp_Y_vel
        self.ki_Y_vel = ki_Y_vel / 100.0
        self.kd_Y_vel = kd_Y_vel * 100.0
    ...
    def predict(self, obs, state_ref=np.asarray([0.0, 0.0, 0.0])):
        ...
        # Error
        if error_Y_pos >= 180.0:
            error_Y_pos -= 360.0
        elif error_Y_pos <= -180.0:
            error_Y_pos += 360.0
        ...
        if (np.fabs(self.accum_error_R_vel * self.ki_R_vel)) > self.antiwindup:
            self.accum_error_R_vel = 0.0

        if (np.fabs(self.accum_error_P_vel * self.ki_P_vel)) > self.antiwindup:
            self.accum_error_P_vel = 0.0

        if (np.fabs(self.accum_error_Y_vel * self.ki_Y_vel)) > self.antiwindup:
            self.accum_error_Y_vel = 0.0

        # Compute output
        P_error_vel = np.asarray([self.kp_R_vel * error_R_vel,
                                  self.kp_P_vel * error_P_vel,
                                  self.kp_Y_vel * error_Y_vel])
        I_error_vel = np.asarray([self.ki_R_vel * self.accum_error_R_vel,
                                  self.ki_P_vel * self.accum_error_P_vel,
                                  self.ki_Y_vel * self.accum_error_Y_vel])
        D_error_vel = np.asarray([self.kd_R_vel * (error_R_vel - self.last_error_R_vel) / ((time.time() - self.time) * 1000.0),
                                  self.kd_P_vel * (error_P_vel - self.last_error_P_vel) / ((time.time() - self.time) * 1000.0),
                                  self.kd_Y_vel * (error_Y_vel - self.last_error_Y_vel) / ((time.time() - self.time) * 1000.0)])
        ...
        PID_actions[0] = - PID_outputs[0] - PID_outputs[1] + PID_outputs[2]
        PID_actions[1] = - PID_outputs[0] + PID_outputs[1] - PID_outputs[2]
        PID_actions[2] = + PID_outputs[0] - PID_outputs[1] - PID_outputs[2]
        PID_actions[3] = + PID_outputs[0] + PID_outputs[1] + PID_outputs[2]
...
class POS_VEL_PID_SIM():
    def __init__(self, env, kp_R_pos, kp_P_pos, kp_Y_pos, kp_R_vel, ki_R_vel, kd_R_vel, kp_P_vel, ki_P_vel,
                 kd_P_vel, kp_Y_vel, ki_Y_vel, kd_Y_vel, f=70.0):
        # Env instance
        self.env = env
    ...
    def predict(self, obs, state_ref=np.asarray([0.0, 0.0, 0.0])):
        ...
        # Compute output
        P_error_pos = np.asarray(
            [self.kp_R_pos * error_R_pos, self.kp_P_pos * error_P_pos, self.kp_Y_pos * error_Y_pos])
        ...
        # Compute output
        P_error_vel = np.asarray(
            [self.kp_R_vel * error_R_vel, self.kp_P_vel * error_P_vel, self.kp_Y_vel * error_Y_vel])
        I_error_vel = np.asarray([self.ki_R_vel * self.accum_error_R_vel, self.ki_P_vel * self.accum_error_P_vel,
                                  self.ki_Y_vel * self.accum_error_Y_vel])
        D_error_vel = np.asarray([self.kd_R_vel * (error_R_vel - self.last_error_R_vel) * self.f,
                                  self.kd_P_vel * (error_P_vel - self.last_error_P_vel) * self.f,
                                  self.kd_Y_vel * (error_Y_vel - self.last_error_Y_vel) * self.f])
...
class POSITION_PID_SIM():
    ...
    def predict(self, obs, state_ref=np.asarray([0.0, 0.0, 0.0])):
        ...
        P_error = np.asarray([self.kp_R * error_R, self.kp_P * error_P, self.kp_Y * error_Y])
        I_error = np.asarray(
            [self.ki_R * self.accum_error_R, self.ki_P * self.accum_error_P, self.ki_Y * self.accum_error_Y])
        D_error = np.asarray(
            [self.kd_R * (error_R - self.last_error_R) * self.f, self.kd_P * (error_P - self.last_error_P) * self.f,
             self.kd_Y * (error_Y - self.last_error_Y) * self.f])
